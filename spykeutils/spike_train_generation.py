...
def gen_homogeneous_poisson(rate, t_start = 0 * pq.s, t_stop = None,
        max_spikes=None):
    ...
    max_duration = t_stop - t_start if t_stop != None else None

    if t_stop == None and max_spikes == None:
    ...
    if max_spikes != None:
        ...
        while spike_times[-1] <= t_stop:
            spike_times.append(spike_times[-1]
                    + numpy.random.exponential(scale) * scale.units)
